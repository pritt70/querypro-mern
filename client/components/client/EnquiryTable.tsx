...
const EnquiryTable = ({
  ...
  const deleteRow = (delid: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${API_BASE_URL}/api/enquiry/enquiryremove/${delid}`)
          .then((res) => {
  ...
  const editRow = (editId: string) => {
    axios
      .get(`${API_BASE_URL}/api/enquiry/enquiryedit/${editId}`)
      .then((res) => {
        const data = res.data.enquiry;
        setFormData(data);
      })
      .catch((err) => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0 mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              >
                {filteredData?.length || 0} of {data?.length || 0} enquiries
              </p>
            </div>
          </div>
